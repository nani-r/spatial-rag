...
def extract_answers(data):
    """Extracts easy difficulty answers and converts them to numbers."""
    answers = []
    ...
    for item in source_data:
        if isinstance(item, dict) and "easy" in item and "answer" in item["easy"]:
            num_answer = extract_numeric_answer(item["easy"]["answer"])
...
fig.text(0.5, 0.95, "Distribution of Residuals", ha='center', fontsize=18, fontweight='bold')
...
plt.savefig("residual_histograms_easy.png", dpi=300)
plt.show()
...
def calculate_mse(residuals, perfect_answers):
    ...
    if valid_residuals:
        ...
        mse = np.nan  # In case all values were NaN
...
with open("mse_report_easy.txt", "w") as file:
